  ...
  <section>
    ...
    <div v-if="!equipos.length">
      ...
      <p class="flex justify-center">
        <NLink class="btn" to="/equipos">Ver Equipos</NLink>
      </p>
    </div>
      ...
      <div>
        <div class="order-3 lg:order-1">
          <div class="flex flex-wrap my-4">
            ...
            >
              <input type="checkbox" v-model="equipo.viendo" class="scale-150 mr-2" />
              {{ equipo.nombre }}
            </div>
          </div>
        </div>
        ...
        <div class="grid proximas gap-2">
          <template v-for="(a, index) of proximasFiltro">
            ...
            >{{ a.detalles.reunion ? 'Detalles' : a.detalles.actividad.titulo }}</NLink>

            <NLink
              v-else
              :to="'/eventos/' + a.detalles.evento.id"
              :key="'v' + index"
            ...
            >{{ a.detalles.evento.titulo }}</NLink>
      ...
      return this.proximas
        .filter(
          a => {
            ...
            const e = this.equipos.find(x => x.id === this.dameEquipoId(a)) 
            return e && e.viendo
          }
        )
        .map(a => {
