  ...
  <div>
   ...
   <section class="flex flex-wrap sm:flex-nowrap justify-between items-baseline mb-5">
      ...
      <form @submit.prevent="buscar" class="w-full sm:w-auto flex justify-end order-1">
        ...
        <button :disabled="buscarPor.length<=1" type="submit" class="ml-2 btn">Buscar<span class="hidden md:inline"> en Usuarios</span></button>
      </form>
    </section>
    <Grid class="grid-cols-fill-w-40">
      <CardUser
        v-for="usuario of usuariosListados"
        :key="usuario.id"
        :data="usuario"
      />
    </Grid>
