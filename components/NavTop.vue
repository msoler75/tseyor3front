  ...
  >
    ...
    <nav
      ...
      class="
        ...
        text-gray-800
        bg-gray-50
        dark:bg-gray-dark-900 dark:text-gray-100
        text-sm
        sm:overflow-visible
      "
    ...
    >
      ...
      <template v-for="item of rutasMenu">
        <div
          ...
          v-if="item.items"
          :key="item.href"
          class="flex p-4 justify-center overflow-y-auto"
        >
          <div
            class="
              mx-auto
          ...
          >
            <template
              v-for="elem of $store.getters.buildRoutes(
...
export default {
  ...
  mounted() {
    if (process.client) window.addEventListener("resize", this.resizeHandler);
  },
  computed: {
    ...mapGetters([
      "travelling",
  ...
  methods: {
    ...
    menuHover(item) {
      if (item && item.items) this.menuClick(item);
      else {
        this.localValue = "";
      }
    },
    ...
    inPath(url) {
      ...
      for (const item of this.rutasMenu) {
        ...
        if (item.items) {
          for (const elem of this.$store.getters.buildRoutes(
            this.getAllDescendants(item.items)
          )) {
    ...
    updateSoftPath() {
      ...
      if (tab) {
        ...
        this.softPath = `M ${x0 - w} ${y + h2} L ${x0} ${y - h1} L ${x1} ${
          y - h1
        } L ${x1 + w} ${y + h2} Z`;
      }
    },
