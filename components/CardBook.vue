<template>
  <Card class="card-book h-full" :class="landscape ? 'landscape' : ''" :clicked="clicked">
    <section class="h-full flex flex-col justify-start" :class="landscape ? 'xm:flex' : ''">
      <section class="sm:pt-2">
        <Book3D class="mx-auto" :class="sizeBook" :src="cimage" />
        <!-- <nuxt-img class="block xs:hidden max-w-8" :src="'/imagenes/portadas/' + data.imagen"/> -->
      </section>
      <section class="px-1 xs:px-6 text-center flex flex-col flex-grow"
        ...
        <v-clamp v-if="!noText && ctext" autoresize :max-lines="5" class="my-auto text-gray-700 text-base">{{ cteaser }}
        </v-clamp>
        <div v-if="showTags" class="px-6 pt-4 pb-2 mt-auto">
          <span v-for="tag of ctags" :key="tag"
